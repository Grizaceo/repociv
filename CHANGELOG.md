...
- **Bug de input isolation en vista local (Phase 6, 3 capas)** — al entrar a la vista local isométrica 2D, los listeners del renderer global (registrados en el mismo `HTMLCanvasElement` que `LocalRenderer`, pero antes y sin guard de `viewMode`) seguían ejecutándose en cada `mousedown`/`mousemove`/`mouseup`/`wheel`/`contextmenu`/`dblclick`/`mouseleave`. El `pickAxial()` global mapeaba las coords de pantalla al grid del mapa de ciudades, y abría paneles de otras ciudades aunque la vista local las tapara. Capa 1: `bailIfLocal()` (helper privado en `renderer.ts`) — los 7 listeners del global consultan `state.viewMode` y el atributo `data-local-active` del canvas antes de hacer cualquier trabajo. Capa 2: `LocalRenderer.setupInput` ahora usa `e.stopImmediatePropagation()` (antes `stopPropagation()`, que solo detiene bubbling a ancestros, no a listeners hermanos en el mismo target). Capa 3: `handleClick` (la función que `mouseup` invoca) tiene un backstop que también consulta ambas señales, así cualquier ruta futura que llegue sin pasar por los listeners queda cubierta; y `render()` mantiene `canvas.dataset.localActive` en sync con `state.viewMode` como segunda señal canal-independiente. 7 tests nuevos en `renderer.test.ts` (mousedown local short-circuited, dataset sync, macro mousedown no roto, macro drag-pan no roto, local drag no pan-ea cámara global, handleClick backstop, localR stopImmediatePropagation). 591 → 598 vitest.
