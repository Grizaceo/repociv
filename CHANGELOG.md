...
- **Ciudades: centro cívico de piedra (iter11, prioridad #3)** — las ciudades eran props (edificios/techos/muros/torres) sueltos sobre el pasto. Ahora cada ciudad se asienta sobre un **dais de piedra escalonado** (cilindro cónico, base 0.46·HEX que rebasa los muros 0.38·HEX → la plataforma enmarca toda la ciudad; taper a 0.30·HEX = terraza cívica, no disco plano) que recibe sombra. Las ciudades normales suman un **obelisco-monumento de mármol** (offset al sur, sobre la plaza, despejado del edificio central); las capitales conservan el keep GLB como landmark y suman **dos obeliscos flanqueantes** como distrito monumental (radio 0.42·HEX, despejados de la huella ~0.27·HEX del keep). Geometrías altas y finas que el sol fijo de iter9 proyecta como sombras largas. Sin tocar el bookkeeping de instancias de edificios (3 meshes nuevos: `plazaMesh`/`spireMesh`/`capitalLandmarkMesh`, counts simples por ciudad). `CityCluster3D.ts`. Verificado before/after (cámaras capital + mid).
- **Ríos por aristas de hex + agua turquesa coherente (iter10, prioridad #2)** — los ribbons de río se ruteaban por CENTROS de tile; ahora siguen el grafo de ARISTAS como en Civ V, sin tocar el shader ni romper el determinismo. Dos hechos de la malla hex regular lo hacen exacto y sin índices frágiles: el punto medio de dos centros adyacentes ES el punto medio de su arista compartida (los centros son espejo a través de esa arista), y tres hexes mutuamente adyacentes comparten un vértice = centroide de los tres centros (giros de 60° ⇒ mid→esquina→mid, ambos tramos sobre aristas reales). Solo la cabecera/manantial conserva el centro del tile fuente. Color del agua pasó de azul `0x3f8ecb` a turquesa `0x4ca1a8` para igualar el tinte costero del shader (`rgb 0.30,0.63,0.66`) — un río ahora lee como la MISMA agua que la costa, no un ribbon azul aparte; specular suave + emissive teal tenue. Boca ajustada para dipear en el agua (`+0.5`, antes `+1.0`; alcance `0.70`, antes `0.62`) y fundir en el océano sin hueco, con espuma en la unión. `Rivers3D.test.ts` verde (determinismo + desembocadura preservados). Verificado con before/after en cámaras centradas en ríos reales (las coords de la cámara golden 07 quedaron obsoletas). `Rivers3D.ts`.
- **Sombras del sol oscilantes (iter9, prioridad #1)** — el `updateHexWorldScene` re-seteaba `sunLight.position` cada frame desde un arco basado en `animTime`; como la cámara ortográfica de sombras se sitúa EN la luz y mira al target, la dirección de luz oscilaba continuamente y las sombras temblaban/nadaban en juego en vivo. Ahora el sol está **fijo a ~35° de elevación** (`SUN_POSITION`, tarde dorada) en el constructor; `updateHexWorldScene` solo respira color/intensidad (shimmer golden-hour, no mueve geometría). El ángulo más bajo alarga las sombras hacia la escena (visibles bajo la cámara inclinada) vs el viejo ~50° de mediodía cuyas sombras cortas se metían bajo cada prop. `normalBias` bajado 1.5 → 0.8 (el valor alto despegaba la sombra del caster, peter-panning, muy visible con sombras largas); `bias` -0.0004 → -0.0005. Props/ciudades/unidades anclan al suelo estilo Civ V. Verificado con captura before/after en el mismo entorno (`HexWorldScene.ts`, `ThreeMapRenderer.ts`). Nota: los goldens comprometidos (`e2e/golden/`) quedan sin tocar — fueron capturados en GPU real y ya estaban desactualizados desde iter8; requieren `--update` en máquina con GPU.
