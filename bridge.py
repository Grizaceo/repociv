...
CONFIG_DIR = Path.home() / ".repociv"
CONFIG_DIR.mkdir(exist_ok=True)
MISSIONS_FILE = CONFIG_DIR / "missions.json"
PENDING_TRACKER = Path.home() / ".hermes" / "workspace" / "PENDING_TRACKER.md"
...
# ─── Agent execution ────────────────────────────────────────────────────────
AGENT_CONFIGS = {
    "DAVI":   {"agent": "main", "personality": "technical"},
    "WORKER": {"agent": "main", "personality": "concise"},
    "SCOUT":  {"agent": "main", "personality": "helpful"},
    "LEXO":   {"agent": "main", "personality": "analytical"},
}


def _get_agent_config(unit_id: str) -> dict[str, str]:
...
def _execute_streaming(unit_id: str, mission_id: str, mission: str) -> tuple[bool, str]:
    ...
    if _has_openclaw():
        try:
            ...
            send_to_repociv({"type": "chat_chunk", "unit": unit_id, "missionId": mission_id, "text": f"[openclaw error: {e}, fallback a Hermes API]\n"})

    return _run_hermes_streaming(unit_id, mission_id, mission)
...
def _run_openclaw_streaming(unit_id: str, mission_id: str, mission: str,
                             config: dict[str, str]) -> tuple[bool, str]:
    cmd = [
        "openclaw", "agent",
        "--agent", config["agent"],
        "--session-id", f"repociv-{unit_id.lower()}",
        "--message", mission,
    ]
...
def _run_hermes_streaming(unit_id: str, mission_id: str, mission: str) -> tuple[bool, str]:
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": (
                "Eres DAVI, un agente que ayuda a Cristóbal a navegar su workspace. "
    ...
    try:
        ...
        msg = (
            f"[Bridge offline mode]\nMisión recibida: {mission}\n"
            f"No pude conectar con openclaw ni con Hermes API.\nDetalle: {e}\n\n"
            f"Para que funcione real:\n"
            f"  - Asegúrate que `openclaw` esté en el PATH, o\n"
            f"  - Levanta Hermes API en {HERMES_URL.replace('/v1/chat/completions', '')}\n"
        )
        for i in range(0, len(msg), 30):
            send_to_repociv({"type": "chat_chunk", "unit": unit_id, "missionId": mission_id, "text": msg[i:i + 30]})
            time.sleep(0.05)
        return True, msg
