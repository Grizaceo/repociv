...
#   ./scripts/dev-start.sh --with-mcp  # start bridge + vite + MCP server (logs → ~/.repociv/logs/mcp.log)
#
...
if [[ "$WITH_MCP" -eq 1 ]]; then
  echo "▶ Iniciando MCP server (stdio, background)…"
  python3 -m server.mcp_server \
    >> "$LOG_DIR/mcp.log" 2>&1 &
  MCP_PID=$!
  echo "  → MCP PID: $MCP_PID  Log: $LOG_DIR/mcp.log"
fi
...
echo "│  Logs:    $LOG_DIR"
echo "│  PIDs:    bridge=$BRIDGE_PID  vite=$VITE_PID${MCP_PID:+  mcp=$MCP_PID}"
