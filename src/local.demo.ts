...
console.log(
  `World: ${localWorld.width}x${localWorld.height} tiles, ${localWorld.rooms.length} rooms, ${localWorld.workbenches.length} workbenches\n`,
);
...
function spawnLocalUnit(
  ...
  outer: for (let ry = startRoom.y + 1; ry < startRoom.y + startRoom.h - 1; ry++) {
...
function localUpdate(dt: number): void {
  for (const unit of localUnits.values()) {
    if (unit.state === 'walking_to_workbench' || unit.state === 'walking_to_room') {
      ...
      const mission = missionQueue.find(
        (m) => m.unitId === unit.id && (m.status === 'working' || m.status === 'walking'),
      );
      if (mission) {
...
console.log(
  `Spawning: ${davi.name} at (${davi.gridX}, ${davi.gridY}) in room "${localWorld.rooms[0]!.label}"`,
);
...
for (let i = 0; i <= STEPS; i++) {
  ...
  const pathLeft =
    davi.path.length - davi.pathIndex - 1 > 0
      ? ` path-left:${davi.path.length - davi.pathIndex - 1}`
      : '';
  const fatigue = `${davi.fatigue.toFixed(0)}%`;
  const speed = `${(BASE_SPEED * davi.effectiveSpeed).toFixed(1)}`;
  console.log(
    `  t=${t.toFixed(1)}s | DAVI: ${state.padEnd(22)} pos=${pos.padEnd(12)}${work}${pathLeft} | ctx:${fatigue} speed:${speed} tiles/s`,
  );
}
