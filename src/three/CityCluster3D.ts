...
// Civic centre (iter11): a stepped stone plaza dais under every city, a
// central monument spire for normal cities (capitals already have the GLB
// keep as their landmark), and a pair of obelisks flanking each capital as
// its monument district.
...
export function rebuildCityClusters(
  ...
  // A short two-radius cylinder reads as a stepped dais (top narrower than
  // base); every city sits on one so the cluster reads as a deliberate civic
  // ground, not props dropped on grass. Normal cities get a slender marble
  // spire as their monument (offset south, into the open plaza, so it clears
  // the central building); capitals skip the spire — the GLB keep is their
  // landmark — and instead get two flanking obelisks as a monument district.
  {
    // Wide tapered dais: bottom radius (0.46) reaches past the perimeter
    // walls (0.38) so the stone platform frames the whole city; the strong
    // taper to 0.30 reads as a stepped civic terrace, not a flat disc.
    ...
    for (const city of cities) {
      ...
      if (!city.isCapital && spireMesh) {
        // Monument spire, offset south into the open plaza, rising above roofs.
        ...
        // Two obelisks flanking the keep (clear of its ~0.27·HEX footprint).
