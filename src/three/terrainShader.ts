...
vec2 terrainMacroUv(float idx, vec2 tileUv, vec2 worldXZ) {
  ...
  // Rotate the world sampling frame ~31° off the hex/camera axes before fract().
  // The fract() tiling seams (the atlas cells are blocky and a few carry crack
  // lattices) otherwise line up into an axis-aligned RECTANGULAR grid that
  // competes with the hex structure (audit 2026-07-16 P2). Off-axis sampling
  // hides the "rectangular" read; the land weight is eased so the seams that
  // remain are softer. Water stays near-fully world-space (continuous sea).
  const mat2 kMacroRot = mat2(0.857, -0.515, 0.515, 0.857); // ~31°
  vec2 worldUv = fract((kMacroRot * worldXZ) / (uHexRadius * scale)
                       + vec2(idx * 0.137, idx * 0.173));
  bool water = (idx > 3.5 && idx < 5.5);
  return mix(tileUv, worldUv, water ? 0.93 : 0.26);
}
...
`;
    ...
    shader.fragmentShader =
      ...
      shader.fragmentShader
        ...
        if (uUseAtlas > 0 && vTopFace > 0.5) {
          ...
          // Grain now comes from CONTINUOUS world-space noise instead of
          // re-sampling the atlas cell through fract(vWorldXZ / const), which
          // stamped the same cell in an axis-aligned rectangular grid whose
          // seams read as a lattice competing with the hex structure
          // (audit 2026-07-16 P2). Two incommensurate octaves break any single
          // period; terrainDetailNoise is seamless so no grid survives.
          if (tidx < 4.5 || (tidx > 5.5 && tidx < 7.5)) {
            vec2 grainOff = vec2(tidx * 7.31, tidx * 5.17);
            float dLum = 0.6 * terrainDetailNoise(vWorldXZ * 2.30 + grainOff)
                       + 0.4 * terrainDetailNoise(vWorldXZ * 3.70 - grainOff);
        ...
        if (vTopFace > 0.5) {
          ...
          if (_gtidx > 3.5 && _gtidx < 4.5) {
            // Wet but STABLE: a broader specular lobe than the old 0.30→0.17
            // mirror finish. That near-mirror roughness over the wave normal
            // map produced a sharp sun glint that aliased into crawling
            // flicker (bloom amplified it). A soft sheen holds steady instead.
            roughnessFactor = mix(0.44, 0.32, vOceanDepth);
          }
        }`,
        )
        ...
        if (uUseNormalAtlas > 0 && vTopFace > 0.5) {
          ...
          // Water: damp the perturbation hard. A low-roughness sea over the
          // high-frequency atlas normal made the specular glint sparkle and
          // flicker; a near-flat sheet keeps one soft, stable sun reflection
          // (the wave feel comes from the vertex displacement + diffuse
          // shimmer, not from crawling specular).
          float _normStrength = (_ntidx > 3.5 && _ntidx < 4.5) ? 0.20 : 1.0;
          nTex *= _normStrength;
  ...
  mat.customProgramCacheKey = () => 'repociv-terrain-v42';
