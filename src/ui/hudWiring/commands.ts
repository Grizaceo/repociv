...
export function registerHudCommands(
  ...
  registerCommands([
    ...
    { id: 'view-toggle', group: 'Vista', label: 'Alternar mapa 2D / 3D', hint: '3', run: () => toggleView() },
    { id: 'hud-mode', group: 'Vista', label: 'Modo Quick / Advanced (HUD)', run: () => toggleHudMode() },
    { id: 'p-approvals', group: 'Panel', label: 'Aprobaciones', hint: 'A', run: () => toggleApprovalPanel() },
    { id: 'p-tasks', group: 'Panel', label: 'Tareas activas', hint: 'F9', run: () => toggleTaskPanel() },
    { id: 'p-timeline', group: 'Panel', label: 'Crónica de eventos', hint: 'F10', run: () => toggleTimelinePanel() },
    { id: 'p-observability', group: 'Panel', label: 'Observabilidad', hint: 'F8', run: () => toggleObservabilityPanel() },
    { id: 'p-replay', group: 'Panel', label: 'Replay de directivas', hint: 'F7', run: () => toggleReplayPanel() },
    { id: 'p-pending', group: 'Panel', label: 'Pendientes del tracker', run: () => togglePendingPanel() },
    { id: 'p-log', group: 'Panel', label: 'Log en vivo', run: () => toggleLogPanel() },
    { id: 'p-harness', group: 'Panel', label: 'Harness', run: () => toggleHarnessPanel() },
    { id: 'p-ledger', group: 'Panel', label: 'Gran Libro', hint: 'F6', run: () => toggleLedger(state) },
    { id: 'p-task-assign', group: 'Panel', label: 'Asignar tareas', hint: 'J', run: () => toggleTaskAssignPanel(() => state.getLocalUnits(), (unitId, task) => state.setLocalUnitTask(unitId, task)) },
    { id: 'p-layers', group: 'Panel', label: 'Capas del mapa', hint: 'H', run: () => toggleLayerPanel() },
    { id: 'p-settings', group: 'Panel', label: 'Configuración', hint: 'F11', run: () => toggleSettingsPanel() },
    { id: 'p-construction', group: 'Panel', label: 'Modo construcción', run: () => toggleConstructionPanel() },
    { id: 'a-screenshot', group: 'Acción', label: 'Captura de pantalla', hint: 'F12', run: () => takeScreenshot(renderer) },
  ]);
