...
// ─── Public API ────────────────────────────────────────────────────────────────
export function isRecoveryPanelOpen(): boolean { return _visible; }
...
function _renderError(msg: string) {
  ...
  body.innerHTML = `
    <div class="rp-error">
      ...
      <button class="rp-btn rp-btn--retry" id="rp-retry">Reintentar</button>
    </div>
  `;
  _getOrCreate().querySelector('#rp-retry')
...
function _render() {
  ...
  body.innerHTML = `
    ...
    ${hasCommand ? `
      <div class="rp-command-block">
        <div class="rp-command-header">
          ...
          <button class="rp-btn rp-btn--copy" id="rp-copy" title="Copiar al portapapeles">
            📋 Copiar
          </button>
        </div>
    ...
    <div class="rp-footer">
      ...
      <button class="rp-btn rp-btn--close" id="rp-close-panel">Cerrar</button>
    </div>
  `;

  // Wire copy button
  _getOrCreate().querySelector('#rp-copy')
    ?.addEventListener('click', () => _copyToClipboard(command ?? ''));

  _getOrCreate().querySelector('#rp-close-panel')
    ?.addEventListener('click', closeRecoveryPanel);
}
...
function _getOrCreate(): HTMLElement {
  ...
  el.innerHTML = `
    <div class="rp-header">
      ...
      <button id="rp-x" title="Cerrar">✕</button>
    </div>
    <div class="rp-body"></div>
  `;
  document.body.appendChild(el);
  el.querySelector('#rp-x')?.addEventListener('click', closeRecoveryPanel);
  _panel = el;
  return el;
}
